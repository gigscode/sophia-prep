-- ============================================================================
-- SOPHIA PREP - DATABASE REFACTOR PROPOSAL
-- ============================================================================
-- Separate tables for exam types and subject categories for better normalization
...
-- ============================================================================
-- Reference exam_type_id instead of exam_type text
...
CREATE INDEX idx_questions_new_topic ON questions_new(topic_id);
...
-- ============================================================================
-- MIGRATION QUERIES (to run after creating new tables)
-- ============================================================================

-- Migrate existing subjects to new structure
...
-- Migrate subject-exam type relationships
...
-- View to get exam types with their subjects
CREATE OR REPLACE VIEW exam_types_with_subjects AS
SELECT 
  et.id,
  et.name,
  et.slug,
  ...
  ARRAY_AGG(s.slug) as subject_slugs,
  COUNT(s.id) as subject_count
FROM exam_types et
LEFT JOIN subject_exam_types set ON et.id = set.exam_type_id
LEFT JOIN subjects_new s ON set.subject_id = s.id AND s.is_active = true
WHERE et.is_active = true
GROUP BY et.id, et.name, et.slug, et.description, et.full_name, 
         et.duration_minutes, et.total_questions, et.passing_score;

-- ============================================================================
-- QUIZ MODES EXPLANATION & UPDATED TABLES
-- ============================================================================

-- ============================================================================
-- UPDATED QUIZ ATTEMPTS TABLE FOR DIFFERENT MODES
-- ============================================================================
...
-- ============================================================================
-- This helps define rules for each quiz mode
...
BEGIN
  ...
  JOIN subjects_new s ON t.subject_id = s.id
  WHERE s.id = ANY(p_subject_ids)
    AND (p_topic_ids IS NULL OR t.id = ANY(p_topic_ids))
...
-- Example 1: Get practice questions for Mathematics (any exam type)
...
-- Example 2: Get JAMB CBT exam questions for Mathematics and Physics
-- SELECT * FROM get_cbt_exam_questions(
--   'jamb',
--   ARRAY['math-subject-uuid', 'physics-subject-uuid']::UUID[],
--   180
-- );
